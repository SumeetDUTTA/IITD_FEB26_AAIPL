...
class QuestioningAgent(object):
    ...
    def build_prompt(
        ...
        tmpl = (
            ...
            '2.  **Question Quality**: The question must be EXTREMELY DIFFICULT but CONCISE. Test deep conceptual understanding efficiently without verbose text.\n'
            ...
            '7.  **Explanation**: The "explanation" field provides a very concise (under 30 words) justification.\n\n'
            "{5}"
...
if __name__ == "__main__":
    ...
    # Check for files in root or assets folder - attempt to satisfy user's file structure
    assets_path = BASE_DIR / "assets"
    if not (assets_path / "topics.json").exists():
        # Fallback to root if not in assets
        assets_path = BASE_DIR
        print(f"📂 Looking for assets in: {assets_path}")

    inc_samples = QuestioningAgent.load_icl_samples(assets_path / "topics_example.json")

    # Load topics.json file.
    with open(assets_path / "topics.json") as f:
        topics = json.load(f)
    ...
    with open(BASE_DIR / "qgen.yaml", "r") as f:
    ...
    for q in question:
        try:
            ...
            prompt = (
                "Extract **ONLY** the topic, question, choices, answer, and explanation as a valid JSON object.\n"
                "**CRITICAL**: You MUST escape all newlines inside strings. Use \\n instead of raw line breaks for multi-line questions.\n"
            ...
            q_fixed, _, _ = agent.agent.generate_response(
                prompt.format(q),
                "You are an expert JSON validator and fixer.",
                max_new_tokens=1024,
                temperature=0.0,
                do_sample=False,
            )
            ...
            try:
                json.loads(q_fixed)
                print(f"✅ Successfully fixed JSON via self-reflection.")
                q = q_fixed
            except json.JSONDecodeError:
                print(f"❌ Failed to fix JSON even after self-reflection.")
        ques.append(q)
    # Save the questions for later analysis
    agent.save_questions(ques, output_path)
    filtered_file_name = output_path.parent / "filtered_questions.json"
    agent.save_questions(agent.filter_questions(ques), filtered_file_name)
    print(f"Saved to {output_path} and {filtered_file_name}!")
